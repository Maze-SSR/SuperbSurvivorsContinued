...
local function getGirlNames()
	...
	local names = {}
	for i = 1, girlNamesCount do
		names[i] = Get_SS_Name("Names" .. tostring(i)) or ("Girl" .. tostring(i))
	end
	return names
end
...
local function getBoyNames()
	...
	local names = {}
	for i = 1, boyNamesCount do
		names[i] = Get_SS_Name("Namb" .. tostring(i)) or ("Boy" .. tostring(i))
	end
	return names
end
...
function GetRandomName(key)
	...
	local result = ZombRand(1, #SurvivorNameTable[key] + 1)
